...
export const MethaneApplicationPage: FC = () => {
  ...
  const handleSaveQuantity = async (reagentId: number, count: number) => {
    const result = await dispatch(saveReagentQuantity({ reagentId, count }));
    if (saveReagentQuantity.fulfilled.match(result)) {
                      ...
                      <td>
                        <div className="d-flex gap-2 flex-wrap">
                          <Button
                            ...
                            size="sm"
                            onClick={() => handleSaveQuantity(item.id, item.count)}
                          >
